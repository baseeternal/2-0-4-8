...
export default function Game2048() {
  ...
  const handleMove = (dir: string) => {
    ...
    const moveRowLeft = (row: number[]) => {
      ...
      for (let i = 0; i < newRow.length - 1; i++) {
        if (newRow[i] === newRow[i+1]) {
          newRow[i] *= 2;
          setScore(prev => {
            const upd = prev + newRow[i];
            if (upd > topScore) setTopScore(upd);
            return upd;
          });
          newRow[i+1] = 0;
        }
    ...
    if (dir === 'up') for (let c = 0; c < SIZE; c++) {
      const col = moveRowLeft(newBoard.map(r => r[c]));
      for (let r = 0; r < SIZE; r++) newBoard[r][c] = col[r];
    }
    if (dir === 'down') for (let c = 0; c < SIZE; c++) {
      const col = moveRowLeft(newBoard.map(r => r[c]).reverse()).reverse();
      for (let r = 0; r < SIZE; r++) newBoard[r][c] = col[r];
    }
